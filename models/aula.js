...
module.exports = {
    ...
    async confereAgenda (idVeiculo, idInstrutor, idAluno, data) {
        const db = await Database()
        
        const agenda = await db.get(` SELECT veiculos_id, instrutores_id, alunos_id, data_hora FROM aulas WHERE veiculos_id = ${idVeiculo} AND instrutores_id = ${idInstrutor} AND alunos_id = ${idAluno} AND data_hora = "${data}"`)
